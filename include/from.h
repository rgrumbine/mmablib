// void from(metricgrid<T> llt, metricgrid<T> mask, T flagval, T nonval,
//                cfsreg<T> x)
...
//
//Jun 17, 1998: Generalize w3ft01 to be more easily system independant
//Minor mods Jun24, 1999
//Major rewrite of algorithm 2/25/2000 for robustness and so that from.h,
//  fromall.h, and fromcfs.h all were similar (as they ought to be!)
...
// Loop over all points on the local grid
  for (outij.j = 0; outij.j < ny; outij.j++ ) {
     for (outij.i = 0; outij.i < nx; outij.i++) {
       dum = nonval;

       outll = locate(outij);      //Find lat long in local grid
